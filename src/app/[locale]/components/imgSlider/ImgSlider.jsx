...
function ImgSlider() {
  ...
  useEffect(() => {
    ...
    for (i = 0; i < reImg.length; i++) {
      reImg[i].className =
        " AllImg opacity-80 border-2 rounded-lg border-zinc-300 opacity-80 object-contain h-12 w-full cursor-pointer";
    }
    reImg[slideIndex].className =
      " AllImg  opacity-100  border-2 rounded-lg border-zinc-400 object-contain h-12 w-full cursor-pointer";
  }, [slideIndex]);
        ...
        <div className="grid grid-cols-1 max-h-24  w-12  pt-12">
          {Imgs.map((img, index) => (
            <img
              key={index}
              id={index}
              className=" AllImg opacity-80 border-2 rounded-lg border-zinc-300  object-contain h-12 w-full cursor-pointer"
              src={img}
