...
export default function Loginui(props: signIn) {
  ...
  async function onSubmit({ username, password, rememberMe }: FormData) {
    const users = { username, password, rememberMe }
    console.log("data:");
    console.log(users);
    // Replace this with a server action or fetch an API endpoint to authenticate
    axios.post("http://localhost:8001/api/authenticate", users,
      {
        'headers': {
  ...
  const solutions = [
    { name: props.Login, description: props.descriptionLogin, href: `/${props.locale}/logIn`, icon: LoginIcon },
    { name: props.signup, description: props.descriptionsignUp, href: '#', icon: CursorArrowRaysIcon, btn: true },
  ]
  const callsToAction = [
    { name: 'Watch demo', href: `/${props.locale}/logIn`, icon: PlayCircleIcon },
  ...
  const Loginuitest = () => {
    return (
      <>
        {loginopen &&
          <>
            <button onClick={() => { setloginopen(!loginopen) }} className="absolute inset-0  z-40 w-full h-screen bg-gray-800/40 cursor-pointer">
            </button>
            <div dir='ltr' className="absolute inset-0  flex dark:bg-gray-900 items-center justify-center">
              <div className="relative z-50 w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                <button onClick={() => { setloginopen(!loginopen) }} type="submit" title="escap" className='absolute right-3 top-3'>
                  <X />
                ...
                <div className="p-20 space-y-8 md:space-y-8 sm:p-8">
                  ...
                  >
                    <div>
                      ...
                      <input   {...register("username")} className="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="username"
                      />
                      {errors.username && (<p className="mt-2 text-sm text-red-600 dark:text-red-500"><span className="font-medium">{errors.username.message}</span></p>)}
                    </div>
                    ...
                    <div>
                      ...
                      {errors.password && (<p className="mt-2 text-sm text-red-600 dark:text-red-500">
                        <span className="font-medium">{errors.password.message}</span></p>)}
                    </div>

                    <div className="flex items-center justify-between">
                      <div className="flex items-start">
                        <div className="flex items-center h-5">
                          <input  {...register("rememberMe")} type="checkbox" className="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-primary-600 dark:ring-offset-gray-800 mr-2" />
          ...
          {open &&
            <>
              ...
              >
                ...
                <div className="w-screen bg-[lightblue] max-w-md flex-auto overflow-hidden rounded-3xl  text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                  <div className="p-4  z-50">
                    {solutions.map((item) => (
                      <>
                        {item.btn ?
                          <div onClick={() => { setloginopen(!loginopen) }} key={item.name} className="group relative flex w-full gap-x-6 rounded-lg p-4 hover:bg-sky-300 ">
                          ...
                          :
                          <a href={item.href} key={item.name} className="group relative flex gap-x-6 rounded-lg p-4 hover:bg-sky-300 ">
                            <div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                              <item.icon aria-hidden="true" className="h-6 w-6 text-gray-600 group-hover:text-indigo-600" />
                            </div>
                            <div>
